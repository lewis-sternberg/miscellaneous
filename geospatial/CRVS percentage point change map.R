...
pal <- colorBin(
  palette = "YlOrRd",
  na.color="#d0cccf",
  bins = c(-50,-25,-0.00000000000001,0.00000000000001,25,50)
)
...
leaflet(data= plot_dat_t) %>%
  ...
  addPolygons(color=pal(plot_dat_t$change)
              ...
              ,popup=paste0(
                ...
                sub(paste0("<b>Change: </b>NA%<br/>"),"",paste0("<b>Change: </b>",round(plot_dat_t$change)," Percentage point","<br/>"))
              )) %>%
  ...
  addLegend(
    ...
    , opacity = 1
    , title="Percentage point change 2000-2018"
